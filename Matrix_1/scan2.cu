...
float* addScan(float* x, float* y, int n) {
    ...
    finalScan << < n / BLOCK_SIZE, BLOCK_SIZE >> > (d_input, d_aux);
...
float* runScan(float* x, int n) {
    ...
    err = cudaMalloc((void**)&d_input, sizeof(float) * n);
    printf("\nError: runScan d_input %s\n", cudaGetErrorString(err));

    err = cudaMalloc((void**)&d_output, sizeof(float) * n);
    printf("\nError:runScan d_output %s\n", cudaGetErrorString(err));

    err = cudaMemcpy(d_input, x, sizeof(float) * n, cudaMemcpyHostToDevice);
    printf("\nError:runScan d_input cpy  %s\n", cudaGetErrorString(err));

    scanKernal << < n / BLOCK_SIZE, BLOCK_SIZE >> > (d_input, d_output);
    err = cudaDeviceSynchronize();
    printf("\nError:runScancudaDeviceSynchronize cpy %s\n", cudaGetErrorString(err));

    err = cudaMemcpy(test, d_output, sizeof(float) * n, cudaMemcpyDeviceToHost);
    printf("\nError:runScan d_output cpy %s\n", cudaGetErrorString(err));
...
void scanMiddle(float* h_input, float* h_output) {
    ...
    float* secondScan = runScan(ends, N / BLOCK_SIZE);
    float* fixSecondScan = fix(secondScan, N / BLOCK_SIZE);
    float* addScans = addScan(firstScan, fixSecondScan, N);
    //for (int i = 0; i < N; i++) printf("\n Final Scan %i, %f \n", i, addScans[i]);

    //float* addScans = recursion(N, firstScan, d_input, d_output);
    //float* ends = getEnds(firstScan, N);
    //printf("\n BO: %f\n", ends[0]);
    //float* scanEnds = runScan(ends, N, d_input, d_output);
